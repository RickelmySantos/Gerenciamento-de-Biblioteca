...
import { debounceTime, distinctUntilChanged, filter, map, Observable, ReplaySubject } from 'rxjs';
...
export class FilterService<T> {
    ...
    filterItems(items: T[], filterFn: (item: T, query: string) => boolean): Observable<T[]> {
